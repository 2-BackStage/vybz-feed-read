...
public class FanFeedReadController {
    ...
    @Operation(
            ...
            description = "팬피드 데이터를 무한스크롤 방식으로 조회합니다. " +
                    ...
                    "writerUuid를 입력하면 특정 버스커의 팬피드만 조회합니다.",
    ...
    @Operation(
            summary = "특정 버스커 팬피드 무한스크롤 조회 API",
            description = "특정 버스커의 팬피드를 무한스크롤 방식으로 조회합니다. " +
                    "size는 한 페이지에 가져올 개수이며, 다음 목록 요청 시에는 lastId에 이전 목록의 마지막 id를 넣어주세요. " +
                    "sortType은 정렬 기준입니다. 'LATEST', 'LIKES', 'COMMENTS' 중 하나를 입력해주세요.",
            tags = {"FAN-FEED-READ-SERVICE"}
    )
    @GetMapping("/fan/busker/{writerUuid}")
    public BaseResponseEntity<ResponseScrollFanFeedDto> getBuskerFanFeeds(
    ...
    @GetMapping("/fan/detail/{fanFeedId}")
